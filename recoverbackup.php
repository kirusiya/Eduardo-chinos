...
<body>
    ...
    <div class="auth-left d-lg-block d-none">
        <div class="d-flex align-items-center flex-column h-100 justify-content-center">
            <img src="<?php echo $website;?>/assets/images/recovervault.jpg" alt="">
        </div>
    </div>
    <div class="auth-right py-32 px-24 d-flex flex-column justify-content-center">
        <div class="max-w-464-px mx-auto w-100">
            <div>
                ...
                <h4 class="mb-12">Enter Your Recovery Phrase</h4>
                <p class="mb-32 text-secondary-light text-lg">Please enter your 12-word recovery phrase to restore access.</p>
            </div>
            <form action="" method="post" enctype="multipart/form-data">
                <div class="row g-2">
                    <!-- Generate 12 input fields -->
                    <?php for ($i = 1; $i <= 12; $i++): ?>
                        <div class="col-4">
                            <div class="icon-field mb-16">
                                <span class="icon top-50 translate-middle-y fw-semibold me-2">
                                    <?php echo $i; ?>.
                                </span>
                                <input type="text" class="form-control h-56-px bg-neutral-50 radius-12 text-center" name="phrase[]" placeholder="">
                            </div>
                        </div>
                    <?php endfor; ?>
                </div>
                <button type="button" id="recover-btn" class="btn btn-primary text-sm btn-sm px-12 py-16 w-100 radius-12 mt-32"> Recover</button>
                <div id="alert-container" class="alert alert-danger mt-3 d-none" role="alert"></div>
            </form>
        </div>
    </div>
</section>
...
<script>
    document.getElementById("recover-btn").addEventListener("click", function() {
        const inputs = document.querySelectorAll("input[name='phrase[]']");
        ...
        if (emptyFields) {
            ...
            alertContainer.textContent = "You must fill in all fields with your secret phrase.";
        } else {
            alertContainer.classList.remove("d-none");
            alertContainer.textContent = "Incorrect phrase, please try again.";
        }
    });
</script>
